...
const EMPTY_FORM = {
  ...
  notas: '', estado: 'confirmado', beneficiario: '',
};
...
function Row({ m, onEdit, onDelete }) {
  return (
    <tr className="border-b border-outline-variant/10 hover:bg-surface-variant/30 transition-colors group">
      ...
      <td className="py-3 px-4 text-sm font-medium max-w-[200px] truncate">{m.descripcion}</td>
...
export default function FinanzasPage() {
  ...
  useEffect(() => { setMovimientos(load()); }, []);

  function persist(next) { setMovimientos(next); save(next); }
  ...
  function handleSave(e) {
    ...
    if (editingId) {
      persist(movimientos.map(m => m.id === editingId ? { ...form, id: editingId } : m));
    } else {
      persist([{ ...form, id: uid(), monto: Number(form.monto) }, ...movimientos]);
    }
    setShowModal(false);
    setEditingId(null);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-on-surface-variant text-sm">Ingresos, egresos y deudas de tu negocio</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <form onSubmit={handleSave} className="bg-surface-container w-full max-w-lg rounded-2xl border border-outline-variant/30 overflow-auto max-h-[90vh]">
            ...
            <div className="flex gap-3 px-6 pb-6">
              {editingId && (
                <button type="button" onClick={() => handleDelete(editingId)} className="text-error hover:bg-error/10 px-3 py-2 rounded-lg transition-colors">
                  <span className="material-symbols-outlined">delete</span>
                </button>
              )}
              <button type="button" onClick={() => setShowModal(false)} className="btn-secondary flex-1">Cancelar</button>
              <button type="submit" className="btn-primary flex-1">
                {editingId ? 'Guardar cambios' : `Registrar ${form.tipo}`}
              </button>
            </div>
          </form>
        </div>
